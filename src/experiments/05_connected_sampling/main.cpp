...
//
// The fixture dimmer now uses the same formula as the background field,
// so the red points pulse in sync with the sine wave beneath them.
// This proves that the shader can drive both the visual field and the
// per‑fixture output from a single spatial function.
//
// Builds on Experiment 04. No new concepts — just connects the two
// previously independent outputs.
...
// Compute shader: RGBA field + fixture output (dimmer matches field)
...
void main() {
    ...
    // Field value (same formula used for fixture dimmer)
    float field_val = sin((nx * 10.0 + u_time) * 3.14159) * 0.5 + 0.5;
    imageStore(u_field, pixel, vec4(field_val, field_val, field_val, 1.0));

    if (gl_LocalInvocationID.x == 0 && gl_LocalInvocationID.y == 0) {
        for (int i = 0; i < 10; i++) {
            float u = u_fixture_uv[i].x;

            // Dimmer uses the SAME formula as the background field
...
int main() {
    ...
    SDL_Window* window = SDL_CreateWindow("Experiment 05 — Connected Sampling",
    ...
    printf("Experiment 05 — Connected Sampling ready.\n");
